...
import { FaUser, FaMapMarkerAlt, FaKey, FaHistory, FaEdit } from "react-icons/fa";
...
export default function Profile() {
  ...
  const handleInfoSave = (e) => {
    ...
    const updatedUser = {
      ...
      user: user?.user ? { ...user.user, name: personalInfo.name, email: personalInfo.email } : undefined,
    };
    login(updatedUser);
    setIsEditing(false);
  ...
  const handlePasswordSave = (e) => {
    ...
    if (!passwordInfo.oldPassword || !passwordInfo.newPassword) {
      toast.error("Please fill in passwords");
      return;
    }
    ...
    <div className="max-w-5xl mx-auto py-16 px-6">
      ...
      <div className="bg-white border border-bakery-cream/35 shadow-sm rounded-3xl p-8 mb-10 flex flex-col md:flex-row items-center gap-6">
        ...
        <div className="text-center md:text-left flex-grow">
          <h1 className="text-3xl font-black font-serif text-bakery-burgundy">{userName}</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-10 items-start">
        ...
        <div className="lg:col-span-1 bg-white border border-bakery-cream/35 rounded-3xl p-6 space-y-2 shadow-sm">
          ...
          >
            <FaMapMarkerAlt className="text-sm" /> Saved Address
          </button>
          <button
          ...
          >
            <FaKey className="text-sm" /> Security Settings
          </button>
        </div>
        ...
        <div className="lg:col-span-3 bg-white border border-bakery-cream/35 rounded-3xl p-8 shadow-sm">
          {activeTab === "info" && (
            <form onSubmit={handleInfoSave} className="space-y-6">
              <h2 className="text-2xl font-bold font-serif text-bakery-burgundy border-b pb-3 mb-6 flex items-center gap-2">
                Personal Information <FaEdit className="text-sm text-bakery-terracotta" />
              </h2>
                ...
                {isEditing ? (
                  <>
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        setPersonalInfo({ name: userName, email: userEmail });
                        setIsEditing(false);
                      }}
          ...
          {activeTab === "address" && (
            <form onSubmit={handleAddressSave} className="space-y-6">
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-xs font-bold text-gray-400 uppercase tracking-wider block mb-1">Contact Phone</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-xs font-bold text-gray-400 uppercase tracking-wider block mb-1">Street Address</label>
                  <textarea
